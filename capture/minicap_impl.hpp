...
private:
  ...
  createVirtualDisplay() {
    ...
    printf("Creating SurfaceComposerClient \n");
    android::sp<android::SurfaceComposerClient> sc = new android::SurfaceComposerClient();

    printf("Performing SurfaceComposerClient init check \n");
    if ((err = sc->initCheck()) != android::NO_ERROR) {
      printf("Unable to initialize SurfaceComposerClient \n");
      return err;
    }
    ...
    printf("Publishing virtual display \n");
...
int minicap_try_get_display_info(int32_t displayId, Minicap::DisplayInfo* info) {
  ...
  if (err !=  android::NO_ERROR) {
    printf("SurfaceComposerClient:::getDisplayState() failed: %s (%d)\n", error_name(err), err);
    return err;
  }
  ...
  if (err !=  android::NO_ERROR) {
    printf("SurfaceComposerClient::getActiveDisplayMode() failed: %s (%d)\n", error_name(err), err);
    return err;
  }
